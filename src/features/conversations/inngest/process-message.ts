...
export const processMessage = inngest.createFunction(
  {
    ...
    onFailure: async ({ event, step }) => {
      const { messageId, conversationId, projectId } = event.data.event
  ...
  async ({ event, step }) => {
    ...
    const conversation = await step.run("get-conversation", async () => {
      return await convex.query(api.systemFunc.getConverstaionById, {
        internalKey: internalKey,
    ...
    //fetch recent messsages of conversation context

    // Fetch recent messages for conversation context
    const recentMessages = await step.run("get-recent-messages", async () => {
      return await convex.query(api.systemFunc.getRecentMessages, {
        internalKey,
        conversationId,
        limit: 10,
      });
      ...
      if (textMessage?.type === "text") {
        ...
        if (title) {
          await step.run("update-conversation-title", async () => {
            await convex.mutation(api.systemFunc.updateConversationTitle, {
              internalKey,
              conversationId,
              title,
            });
    ...
    const codingAgent = createAgent({
      ...
      tools: [
        createReadFilesTool({ internalKey: internalKey }),
